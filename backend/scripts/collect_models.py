...
# 导入项目模块
from app.dao.config_dao import ConfigDAO  # noqa: E402
from app.services.external.crawler.cargurus_crawler import (
    CarGurusCrawler,
)  # noqa: E402
from app.utils.business.profile_utils import (
    generate_daily_profile_name,
)  # noqa: E402
from app.utils.core.logger import logger  # noqa: E402
from app.utils.core.path_util import (
    get_cargurus_data_dir,
    get_data_dir,
)  # noqa: E402
from app.utils.web.browser_utils import browser_utils  # noqa: E402


async def get_brand_code(brand_name: str) -> str:
    """获取品牌代码，用于CSV文件中的make_code字段"""
    # 使用ConfigDAO中的get_make_code函数
    config_dao = ConfigDAO()
    return await config_dao.get_make_code(brand_name)


# 从ConfigDAO获取品牌列表
config_dao = ConfigDAO()
BRANDS = config_dao.get_all_makes()


async def collect_models_for_brands(
    make: str = None,
    city: str = "toronto",
    delay: int = 3,
    ...
    try:
        # 获取城市ZIP代码
        zip_codes = config_dao.get_city_zip_codes(city)
        if not zip_codes:
        ...
        if make:
            if make.lower() in [brand.lower() for brand in BRANDS]:
                brands_to_collect = [
                    brand for brand in BRANDS if brand.lower() == make.lower()
                ]
            else:
                logger.log_result(
                    "错误", f"品牌 '{make}' 不在支持的品牌列表中"
                )
                return {}
        else:
        ...
        logger.log_result(
            "开始收集", f"城市: {city}, ZIP: {zip_code}, 延迟: {delay}秒"
        )
        logger.log_result(
            "品牌列表", f"将收集 {len(brands_to_collect)} 个品牌的车型数据"
        )
        ...
        async with browser_utils.get_driver(profile_name) as driver:
            for i, brand_name in enumerate(brands_to_collect):
                ...
                if output_file.exists():
                    logger.log_result(
                        "跳过收集", f"{brand_name} 的CSV文件已存在，跳过收集"
                    )
                    continue
                ...
                try:
                    # 使用已存在的driver收集车型数据
                    models = await crawler.model_collector.collect_models_for_brand_with_driver(
                    ...
                    if models:
                        ...
                        logger.log_result(
                            "文件保存", f"{brand_name} 数据已保存到CSV文件"
                        )
                    else:
                        logger.log_result(
                            "车型收集", f"{brand_name}: 无车型数据"
                        )

                    # 添加延迟避免被封
                    if (
                        i < len(brands_to_collect) - 1
                    ):  # 最后一个品牌不需要延迟
                    ...
                    logger.log_result(
                        "车型收集失败", f"{brand_name}: {str(e)}"
                    )
                    continue

        logger.log_result(
            "收集完成", f"总共收集到 {len(all_models)} 个品牌的车型数据"
        )
        return all_models
...
async def save_single_brand_to_csv(
    ...
    try:
        ...
        # 获取品牌代码
        brand_code = await get_brand_code(brand_name)
...
async def save_models_to_csv(
    ...
    try:
        ...
        for brand_name, models in models_data.items():
            ...
            brand_code = await config_dao.get_make_code(brand_name)
            # 使用品牌名称的小写形式作为文件名
            ...
            saved_files.append(
                f"{brand_name.title()}: {len(csv_data)} 条记录 -> "
                f"{output_file.name}"
            )
        ...
        logger.log_result("CSV文件保存失败", f"保存车型数据失败: {str(e)}")
        return False
...
async def main():
    ...
    parser.add_argument(
        "--make", help="指定品牌名称，如果不指定则收集所有品牌"
    )
    parser.add_argument("--city", default="toronto", help="城市名称")
    parser.add_argument(
        "--delay", type=int, default=3, help="品牌间延迟时间（秒）"
    )
    ...
    models_data = await collect_models_for_brands(
        make=args.make, city=args.city, delay=args.delay, output_dir=output_dir
    )
