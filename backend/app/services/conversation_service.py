...
class ConversationService:
    ...
    async def process_message(self, request: ConversationRequest) -> ConversationResponse:
        ...
        """
        # 关键部位日志：主要业务函数入口
        logger.log_result("开始对话处理", f"用户消息: {request.message[:50]}...")
        ...
        try:
            ...
            # 关键部位日志：状态变化 - 对话完成
            logger.log_result("对话处理完成", f"AI回复: {ai_response['message'][:50]}...")
    ...
    async def _generate_ai_response(self, user_message: str, conversation_history: List[ConversationMessage]) -> Dict:
        """
        使用Gemini生成AI回复
        """
        # 关键部位日志：外部调用 - AI API
...
"""
    ...
    def _build_conversation_context(self, conversation_history: List[ConversationMessage]) -> str:
        """
        构建对话上下文
        """
